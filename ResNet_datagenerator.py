...
class ImageDataGenerator(object):
    ...
    def __init__(self, txt_file, mode, batch_size, num_classes, shuffle=True,
                 buffer_size=1000, size=None, fulval=True, pos_x = 1, pos_y = 1):
    ...
    def _parse_function_inference(self, filename, label):
        ...
        img_decoded = img_decoded[:, :, ::-1]
        img_decoded = tf.subtract(tf.to_float(img_decoded), IMAGENET_MEAN_FULL)
        img_bgr = img_decoded[crop[self.pos_x]: crop[self.pos_x] + image_height, crop[self.pos_y]: crop[self.pos_y] + image_width]
        return img_bgr, one_hot